...
def salvar_tempos_simulado():
    ...
    try:
        ...
        return jsonify({'status': 'erro', 'mensagem': 'Erro interno ao salvar os dados.'}), 500import os
from flask import Flask, render_template, jsonify, request
from flask_sqlalchemy import SQLAlchemy
...
# --- CLASSE QUE FOI MOVIDA PARA O LUGAR CORRETO ---
class TemposSimulado(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    aluno_id = db.Column(db.Integer, db.ForeignKey('alunos.id'), nullable=False)
    simulado_id = db.Column(db.Integer, db.ForeignKey('simulados.id'), nullable=False)
...
# --- ROTA DE SETUP ATUALIZADA (PARA POPULAR O BANCO VAZIO) ---
@app.route('/_iniciar_banco_de_dados_uma_vez')
def iniciar_banco():
    try:
