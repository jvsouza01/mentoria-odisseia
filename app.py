...
# --- ROTA ADMINISTRATIVA TEMPORÁRIA PARA RENOMEAR ALUNO ---
# ATENÇÃO: USAR APENAS UMA VEZ E DEPOIS REMOVER POR SEGURANÇA
@app.route('/_admin/renomear_aluno')
def admin_renomear_aluno():
    # Pegamos os nomes da URL (?nome_antigo=...&nome_novo=...)
    nome_antigo = request.args.get('nome_antigo')
    nome_novo = request.args.get('nome_novo')

    if not nome_antigo or not nome_novo:
        return "Erro: Forneça 'nome_antigo' e 'nome_novo' como parâmetros na URL.", 400

    try:
        # Tenta encontrar o aluno com o nome antigo
        aluno = Alunos.query.filter_by(nome=nome_antigo).first()

        if not aluno:
            return f"Erro: Aluno '{nome_antigo}' não encontrado.", 404

        # Verifica se o novo nome já não está em uso por OUTRO aluno
        existe_novo = Alunos.query.filter(Alunos.nome == nome_novo, Alunos.id != aluno.id).first()
        if existe_novo:
            return f"Erro: O nome '{nome_novo}' já está em uso por outro aluno.", 409

        # Atualiza o nome e salva no banco
        aluno.nome = nome_novo
        db.session.commit()

        return f"Sucesso! Aluno '{nome_antigo}' foi renomeado para '{nome_novo}'.", 200
